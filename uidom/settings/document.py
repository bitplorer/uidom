        ...
        root_dir = (
            (
                ...
                if os.path.isdir(file_settings.BASE_DIR)
                    ...
                    file_settings.BASE_DIR.replace(".", os.path.sep), subdir
                )
                ...
                if file_settings.BASE_DIR != "__main__"
                    ...
                    os.path.dirname(sys.modules["__main__"].__file__), subdir
                )
            )
            if isinstance(file_settings.BASE_DIR, str)
            else os.path.join(file_settings.BASE_DIR, subdir)
            if any(subdir)
            else file_settings.BASE_DIR
        )
...
class UrlConfig(object):
    def __init__(self, dir_config: DirConfig):
        ...
        self.TEMPLATE_URL = Path(os.path.sep) / dir_config.TEMPLATE_DIR.relative_to(
            dir_config.ROOT_DIR
        )
        self.STATIC_URL = Path(os.path.sep) / dir_config.STATIC_DIR.relative_to(
            dir_config.ROOT_DIR
        )
        self.UPLOAD_URL = Path(os.path.sep) / dir_config.UPLOAD_DIR.relative_to(
            dir_config.ROOT_DIR
        )

        self.STATIC_MEDIA_URL = Path(
            os.path.sep
        ) / dir_config.STATIC_MEDIA_DIR.relative_to(dir_config.STATIC_DIR)
        self.STATIC_FILE_URL = Path(
            os.path.sep
        ) / dir_config.STATIC_FILE_DIR.relative_to(dir_config.STATIC_DIR)

        self.STATIC_CSS_URL = Path(os.path.sep) / dir_config.STATIC_CSS_DIR.relative_to(
            dir_config.STATIC_FILE_DIR
        )
        self.STATIC_JS_URL = Path(os.path.sep) / dir_config.STATIC_JS_DIR.relative_to(
            dir_config.STATIC_FILE_DIR
        )
        self.STATIC_FONT_URL = Path(
            os.path.sep
        ) / dir_config.STATIC_FONT_DIR.relative_to(dir_config.STATIC_FILE_DIR)
        self.STATIC_IMAGE_URL = Path(
            os.path.sep
        ) / dir_config.STATIC_IMAGE_DIR.relative_to(dir_config.STATIC_MEDIA_DIR)
        self.STATIC_AUDIO_URL = Path(
            os.path.sep
        ) / dir_config.STATIC_AUDIO_DIR.relative_to(dir_config.STATIC_MEDIA_DIR)
        self.STATIC_VIDEO_URL = Path(
            os.path.sep
        ) / dir_config.STATIC_VIDEO_DIR.relative_to(dir_config.STATIC_MEDIA_DIR)

        self.UPLOAD_MEDIA_URL = Path(
            os.path.sep
        ) / dir_config.UPLOAD_MEDIA_DIR.relative_to(dir_config.UPLOAD_DIR)
        self.UPLOAD_IMAGE_URL = Path(
            os.path.sep
        ) / dir_config.UPLOAD_IMAGE_DIR.relative_to(dir_config.UPLOAD_MEDIA_DIR)
        self.UPLOAD_AUDIO_URL = Path(
            os.path.sep
        ) / dir_config.UPLOAD_AUDIO_DIR.relative_to(dir_config.UPLOAD_MEDIA_DIR)
        self.UPLOAD_VIDEO_URL = Path(
            os.path.sep
        ) / dir_config.UPLOAD_VIDEO_DIR.relative_to(dir_config.UPLOAD_MEDIA_DIR)
        self.UPLOAD_FILE_URL = Path(
            os.path.sep
        ) / dir_config.UPLOAD_FILE_DIR.relative_to(dir_config.UPLOAD_DIR)

        self.SQL_DATABASE_URL = Path(
            os.path.sep
        ) / dir_config.SQL_DATABASE_DIR.relative_to(dir_config.ROOT_DIR)
    ...
    def dict(self):
        return {item: self.__dict__[item] for item in self.__dict__ if "URL" in item}
...
class DatabaseFile(object):
    def __init__(self, dir_config: DirConfig):
        self.dir: Path = dir_config.DATABASE_DIR
...
class WebAssets(DirConfig):
    ...
    def __init__(self, file_settings: FileSettings):
        super().__init__(file_settings=file_settings)
        # basic settings
        self.dir = self.ROOT_DIR
        # url settings
        self.url = UrlConfig(dir_config=self)

        # files
        self.template = TemplateFile(dir_config=self)
