            ...
            document.querySelectorAll('[x-component]').forEach(component => {
                ...
                class XComponent extends HTMLElement {
                    ...
                    async connectedCallback() {
                        ...
                        if (!!isShadowRoot){
                            ...
                            document.addEventListener('alpine:initialized', () => {
                                ...
                                const child = this.querySelector('[x-data]');
                                const childData = Alpine.$data(child)
                                for (const attr in data) {
                                    childData[attr] = data[attr];
                                }
                    ...
                    update_alpine_elements() {
                        this.childNodes.forEach((element) => {
                        const childrenWithData = element.querySelectorAll('x-data');
                        console.log("safjsajfsajfdn");
                        childrenWithData.forEach((child) => {
                        // Get the data for the child element
                        // replace this with your new data
                        // Update the x-data attribute of the child element
                        child_data = JSON.parse(child.getAttribute('x-data'))
                        if (typeof child_data === "[object Object]"){
                            child.setAttribute('x-data', JSON.stringify({...child.getAttribute('x-data'), ...this.data()}));    
                            }
                        });                        
                        })
                    }
                    ...
                    data() {
                        return Object.fromEntries(this.stateData());
                    }
